...
export function requiresPersonSelection(
  ...
  return PERSON_SPECIFIC_PARAMETERS.includes(
    parameter as PersonSpecificParameter,
  );
}
...
export function isChangeableParameter(parameter: string): boolean {
  return !NON_CHANGEABLE_PARAMETERS.includes(
    parameter as NonChangeableParameter,
  );
}
 ...
 */
export function getParameterCategory(
  parameter: string,
): "person" | "household" | "flexible" | "non-changeable" {
  if (NON_CHANGEABLE_PARAMETERS.includes(parameter as NonChangeableParameter)) {
    return "non-changeable";
  }
  if (
    PERSON_SPECIFIC_PARAMETERS.includes(parameter as PersonSpecificParameter)
  ) {
    return "person";
  }
 ...
 */
export function getPersonSpecificParameters(
  householdMode: "single" | "couple",
): ParameterMetadata[] {
  if (householdMode === "single") {
    return [];
