...
function getEventInfo(
  ...
  const originalIndex = allStates.findIndex((s) =>
...
function DateCell(
  { state, eventInfo }: {
    state: FinancialState;
    eventInfo: ReturnType<typeof getEventInfo>;
  },
) {
  ...
  return (
    <td class="text-gray-900 font-medium">
      <div class={eventInfo.hasEvent ? "font-bold" : ""}>
      ...
      {eventInfo.milestonesAtThisDate &&
        eventInfo.milestonesAtThisDate.map((milestone) => {
          const colors = getMilestoneColors(milestone.category);
          return (
            <div
              key={milestone.id}
              class={`text-xs font-bold mt-1 flex items-center px-2 py-1 rounded ${colors.bg} ${colors.text}`}
            >
        ...
        <thead class="sticky top-0 z-10 bg-gray-50 shadow-sm">
          <tr>
            ...
            <th class="text-right sticky-header bg-gray-50">
              ...
              <div class="text-xs font-normal text-gray-500 mt-1">
                {formatCurrency(
                  (finalState?.investments || 0) +
                    (finalState?.superannuation || 0) +
                    (finalState?.propertyValue || 0),
                )}
              </div>
            </th>
            <th class="text-right sticky-header bg-gray-50">
...
export function LoansTable(
  ...
  const finalState = states[states.length - 1];
  const filteredInterestSaved = states.reduce(
          ...
          {states.map((state, index) => {
            ...
            return (
              <tr key={index} class={eventInfo.rowClass}>
                ...
                <td
                  class={`text-right font-semibold ${
                    effectiveDebt > 0 ? "text-gray-900" : "text-green-600"
                  }`}
                >
        ...
        <thead class="sticky top-0 z-10 bg-gray-50 shadow-sm">
          <tr>
            ...
            <th class="text-right sticky-header bg-gray-50">
              <div>Property</div>
              <div class="text-xs font-normal text-gray-500 mt-1">
                {formatCurrency(finalState?.propertyValue || 0)}
              </div>
            </th>
            <th class="text-right sticky-header bg-gray-50">
              <div>Total Assets</div>
          ...
          {states.map((state, index) => {
            ...
            return (
              <tr key={index} class={eventInfo.rowClass}>
                ...
                <td class="text-gray-900 text-right">
                  {formatCurrency(state.propertyValue || 0)}
                </td>
...
export function TaxTable(
  ...
  const finalState = states[states.length - 1];
  const filteredTaxPaid = states.reduce(
    (sum, state) => sum + (state.periodTaxPaid || 0),
    0,
  );
  const filteredDeductibleInterest = states.reduce(
      ...
      <table class="data-table">
        ...
        <tbody>
          {states.map((state, index) => {
            ...
            const estimatedTaxBenefit = (state.periodDeductibleInterest || 0) *
              0.3;
            ...
            return (
              <tr key={index} class={eventInfo.rowClass}>
                ...
                <td class="text-red-700 text-right font-medium">
                  {index === 0 ? "-" : formatCurrency(state.periodTaxPaid || 0)}
                </td>
                ...
                <td class="text-green-700 text-right">
                  {index === 0 ? "-" : formatCurrency(estimatedTaxBenefit)}
                </td>
              </tr>
            );
          })}
...
export function CashFlowTable(
  ...
  const finalState = states[states.length - 1];
  const filteredExpenses = states.reduce(
    (sum, state) => sum + (state.periodExpenses || 0),
    0,
  );
  const filteredCashFlow = states.reduce(
    (sum, state) => sum + (state.periodCashFlow || 0),
    0,
  );
  const finalCashAvailable = (finalState?.cash || 0) +
    (finalState?.offsetBalance || 0);
