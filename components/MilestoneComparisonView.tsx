...
export default function MilestoneComparisonView(
  { comparison }: MilestoneComparisonViewProps,
) {
    ...
    return (
      <span
        class={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
          colors[effect as keyof typeof colors] || colors.no_change
        }`}
      >
        {effect.replace("_", " ")}
      </span>
              ...
              >
                ...
                <p
                  class={`text-lg font-semibold ${
                    getTimingColor(diff.averageTimingDifference)
                  }`}
                >
                ...
                <div class="flex items-start justify-between mb-3">
                  <div class="flex items-center">
                    <span class="text-2xl mr-3">
                      {getMilestoneIcon(milestone.withTransitions)}
                    </span>
                    <div>
                  ...
                  <div class="text-right">
                    <p
                      class={`text-lg font-semibold ${
                        getTimingColor(milestone.timingDifferenceInDays)
                      }`}
                    >
                    ...
                    {milestone.impactDifference !== undefined && (
                      <p class="text-sm text-gray-600">
                        Impact: {milestone.impactDifference > 0 ? "+" : ""}
                        {formatCurrency(milestone.impactDifference)}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  ...
                  <div class="bg-blue-50 p-3 rounded-md border border-blue-200">
                    ...
                    <p class="text-sm text-gray-700 mb-1">
                      <strong>Date:</strong>{" "}
                      {milestone.withTransitions.date.toLocaleDateString()}
                    </p>
                    {milestone.withTransitions.financialImpact !== undefined &&
                      (
                        <p class="text-sm text-gray-700 mb-1">
                          <strong>Impact:</strong> {formatCurrency(
                            milestone.withTransitions.financialImpact,
                          )}
                        </p>
                      )}
                  ...
                  <div class="bg-gray-50 p-3 rounded-md border border-gray-200">
                    ...
                    <p class="text-sm text-gray-700 mb-1">
                      <strong>Date:</strong>{" "}
                      {milestone.withoutTransitions.date.toLocaleDateString()}
                    </p>
                    {milestone.withoutTransitions.financialImpact !==
                        undefined && (
                      <p class="text-sm text-gray-700 mb-1">
                        <strong>Impact:</strong> {formatCurrency(
                          milestone.withoutTransitions.financialImpact,
            ...
            {comparison.uniqueToWithTransitions.length > 0 && (
              <div>
                <h4 class="font-medium text-blue-800 mb-3 flex items-center">
                  ...
                  Only with Transitions ({comparison.uniqueToWithTransitions
                    .length})
                </h4>
                <div class="space-y-3">
                  {comparison.uniqueToWithTransitions.map((
                    milestone,
                    index,
                  ) => (
                    <div
            ...
            {comparison.uniqueToWithoutTransitions.length > 0 && (
              <div>
                <h4 class="font-medium text-gray-800 mb-3 flex items-center">
                  ...
                  Only without Transitions ({comparison
                    .uniqueToWithoutTransitions.length})
                </h4>
                <div class="space-y-3">
                  {comparison.uniqueToWithoutTransitions.map((
                    milestone,
                    index,
                  ) => (
                    <div
      ...
      {comparison.commonMilestones.length === 0 &&
        ...
        <div class="card p-6 text-center">
          ...
          <h3 class="text-lg font-medium text-gray-600 mb-2">
            No Milestones to Compare
          </h3>
          <p class="text-gray-500">
            No significant financial milestones were detected in either scenario
