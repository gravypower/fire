      ...
      <div class="flex items-start justify-between mb-3">
        <div class="flex items-center gap-3">
          <span
            class="text-2xl"
            role="img"
            aria-label={`${advice.category} advice`}
          >
          ...
          <div class="flex-1">
            ...
            <div class="flex items-center gap-2">
              <span
                class={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${categoryColors.badge}`}
              >
                {advice.category}
              </span>
              <span
                class={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${priorityColors.badge}`}
              >
          ...
          <div>
            <div class="flex items-center justify-between mb-1">
              ...
              <span class="font-medium">
                {Math.floor(advice.effectivenessScore)}%
              </span>
            </div>
...
function RetirementAdvicePanelCore({
  ...
  if (
    !advice || !advice.recommendations || advice.recommendations.length === 0
  ) {
    // Check if we have simulation data to determine if this is a data issue
    if (
      !_currentScenario || !_currentScenario.states ||
      _currentScenario.states.length < 2
    ) {
      return (
        <InsufficientDataDisplay
          onRetry={onRetry}
  ...
  const availableCategories = Array.from(
    new Set(advice.recommendations.map((item) => item.category)),
  );
  ...
  const availablePriorities = Array.from(
    new Set(advice.recommendations.map((item) => item.priority)),
  );
  ...
  return (
    <div class="card p-6">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-3">
          <span class="text-sm text-gray-500">
            ...
            recommendation{advice.recommendations.length !== 1 ? "s" : ""}
          </span>
          {generationErrors.length > 0 && (
            <div class="flex items-center gap-1 text-blue-600">
              ...
              <span class="text-xs">
                {generationErrors.filter((e) => e.severity === "warning")
                  .length} warning{generationErrors.filter((e) =>
                    e.severity === "warning"
                  ).length !== 1
                  ? "s"
                  : ""}
      ...
      <div class="rounded-lg p-3 mb-6 bg-gray-50 border border-gray-200 flex items-start gap-2">
        <span class="text-base leading-none" role="img" aria-label="info">
          ℹ️
        </span>
        <p class="text-xs text-gray-600">
          These recommendations are automatically generated from your simulation
          inputs for general educational purposes only. They are not
          personalized financial advice. Consider your full financial picture
          and consult a licensed financial advisor before acting on them.
        </p>
      </div>

      {/* Overall Assessment */}
      <div
        class={`rounded-lg p-4 mb-6 ${assessmentColors.bg} border ${
          assessmentColors.badge.includes("green")
            ? "border-green-200"
            : assessmentColors.badge.includes("yellow")
            ? "border-yellow-200"
            : "border-red-200"
        }`}
      >
        <div class="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <span
              class={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${assessmentColors.badge}`}
            >
        ...
        <div class="space-y-2 text-sm">
          <div class="flex items-center gap-2">
            <span
              class={advice.retirementFeasibility.canRetireAtTarget
                ? "text-green-600"
                : "text-red-600"}
            >
          ...
          {advice.retirementFeasibility.shortfallAmount && (
            <div class="flex items-center gap-2">
              ...
              <span class={assessmentColors.text}>
                Shortfall:{" "}
                {formatCurrency(advice.retirementFeasibility.shortfallAmount)}
              </span>
            </div>
          )}
          ...
          {advice.retirementFeasibility.surplusAmount && (
            <div class="flex items-center gap-2">
              ...
              <span class={assessmentColors.text}>
                Surplus:{" "}
                {formatCurrency(advice.retirementFeasibility.surplusAmount)}
              </span>
            </div>
          )}
        </div>
        ...
        <div class="mb-6">
          ...
          <div class="space-y-3">
            {advice.quickWins.slice(0, 3).map((item) => {
              const rankedItem = advice.recommendations.find((r) =>
                r.id === item.id
              ) as RankedAdvice;
      ...
      <div class="mb-4">
        <div class="flex flex-wrap gap-2 items-center">
          ...
          <select
            ...
            onChange={(e) =>
              setSelectedCategory(
                (e.target as HTMLSelectElement).value as AdviceCategory | "all",
              )}
          ...
          <select
            ...
            onChange={(e) =>
              setSelectedPriority(
                (e.target as HTMLSelectElement).value as AdvicePriority | "all",
              )}
            class="text-xs border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
      ...
      {advice.longTermStrategies.length > 0 && (
        <div class="mt-6 pt-6 border-t border-gray-200">
          ...
          <div class="text-sm text-gray-600 mb-3">
            These strategies require more planning but offer significant
            long-term benefits.
          </div>
          <div class="space-y-3">
 ...
 */
export default function RetirementAdvicePanel(
  props: RetirementAdvicePanelProps,
) {
  return (
    <AdviceErrorBoundary
