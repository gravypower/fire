...
import type { AdviceCategory, AdviceItem } from "../types/milestones.ts";
 ...
 */
export default function AdviceComparisonView(
  { comparison }: AdviceComparisonViewProps,
) {
  ...
  const formatImpact = (impact: any): string => {
    ...
    if (impact.additionalAssets) {
      parts.push(
        `${formatCurrency(impact.additionalAssets)} additional assets`,
      );
    }
      ...
      <div class="grid grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <p class="text-gray-600">
            Feasibility:{" "}
            <span class="font-medium">
              {Math.floor(advice.feasibilityScore)}%
            </span>
          </p>
        </div>
        <div>
          ...
          <p class="text-xs text-gray-700">
            {formatImpact(advice.projectedImpact)}
          </p>
        </div>
      </div>
    </div>
      ...
      <div class="mb-4">
        <div class="flex items-center mb-2">
          <span class="text-lg mr-2">
            {getCategoryIcon(change.withTransitions.category)}
          </span>
          <h5 class="font-medium text-gray-800">
            {change.withTransitions.title}
          </h5>
        </div>
        ...
        <div class="bg-blue-50 p-3 rounded-md border border-blue-200">
          ...
          <div class="space-y-1 text-sm">
            <div class="flex justify-between">
              ...
              <span
                class={`px-2 py-0.5 rounded text-xs ${
                  getPriorityColor(change.withTransitions.priority)
                }`}
              >
            ...
            <div class="flex justify-between">
              ...
              <span class="font-medium">
                {Math.floor(change.withTransitions.effectivenessScore)}%
              </span>
            </div>
            <div class="flex justify-between">
              <span>Feasibility:</span>
              <span class="font-medium">
                {Math.floor(change.withTransitions.feasibilityScore)}%
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-50 p-3 rounded-md border border-gray-200">
          ...
          <div class="space-y-1 text-sm">
            <div class="flex justify-between">
              ...
              <span
                class={`px-2 py-0.5 rounded text-xs ${
                  getPriorityColor(change.withoutTransitions.priority)
                }`}
              >
            ...
            <div class="flex justify-between">
              <span>Effectiveness:</span>
              <span class="font-medium">
                {Math.floor(change.withoutTransitions.effectivenessScore)}%
              </span>
            </div>
            <div class="flex justify-between">
              <span>Feasibility:</span>
              <span class="font-medium">
                {Math.floor(change.withoutTransitions.feasibilityScore)}%
              </span>
            </div>
          </div>
        </div>
          ...
          <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
            ...
            <div class="space-y-2">
              <p class="text-sm">
                ...
                <span
                  class={`ml-2 font-semibold capitalize ${
                    getAssessmentColor(
                      comparison.withTransitionsAdvice.overallAssessment,
                    )
                  }`}
                >
                  {comparison.withTransitionsAdvice.overallAssessment.replace(
                    "_",
                    " ",
                  )}
                </span>
              ...
              <p class="text-sm">
                ...
                <span
                  class={`ml-2 font-semibold ${
                    comparison.withTransitionsAdvice.retirementFeasibility
                        .canRetireAtTarget
                      ? "text-green-600"
                      : "text-red-600"
                  }`}
              ...
              <p class="text-sm text-gray-600">
                ...
                {comparison.withTransitionsAdvice.recommendations.length}
              </p>
            </div>
          </div>
            ...
            <div class="space-y-2">
              <p class="text-sm">
                ...
                <span
                  class={`ml-2 font-semibold capitalize ${
                    getAssessmentColor(
                      comparison.withoutTransitionsAdvice.overallAssessment,
                    )
                  }`}
                >
                  {comparison.withoutTransitionsAdvice.overallAssessment
              ...
              <p class="text-sm">
                ...
                <span
                  class={`ml-2 font-semibold ${
                    comparison.withoutTransitionsAdvice.retirementFeasibility
                        .canRetireAtTarget
                      ? "text-green-600"
                      : "text-red-600"
                  }`}
              ...
              <p class="text-sm text-gray-600">
                Total Recommendations:{" "}
                {comparison.withoutTransitionsAdvice.recommendations.length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div key={index} class="card p-6">
          <h3 class="text-lg font-semibold mb-4 text-gray-800 flex items-center">
            <span class="text-2xl mr-2">
              {getCategoryIcon(categoryDiff.category)}
            </span>
            {categoryDiff.category.charAt(0).toUpperCase() +
              categoryDiff.category.slice(1)} Advice Differences
          </h3>
              ...
              {categoryDiff.uniqueToWithTransitions.length > 0 && (
                <div>
                  <h4 class="font-medium text-blue-800 mb-3 flex items-center">
                    ...
                    Only with Transitions ({categoryDiff.uniqueToWithTransitions
                      .length})
                  </h4>
                  <div class="space-y-3">
                    {categoryDiff.uniqueToWithTransitions.map((
                      advice,
                      adviceIndex,
                    ) => (
                      <div key={adviceIndex}>
              ...
              {categoryDiff.uniqueToWithoutTransitions.length > 0 && (
                <div>
                  <h4 class="font-medium text-gray-800 mb-3 flex items-center">
                    ...
                    Only without Transitions ({categoryDiff
                      .uniqueToWithoutTransitions.length})
                  </h4>
                  <div class="space-y-3">
                    {categoryDiff.uniqueToWithoutTransitions.map((
                      advice,
                      adviceIndex,
                    ) => (
                      <div key={adviceIndex}>
      ...
      {comparison.adviceDifferences.length === 0 && (
        <div class="card p-6 text-center">
          ...
          <h3 class="text-lg font-medium text-gray-600 mb-2">
            Similar Advice Across Scenarios
          </h3>
          <p class="text-gray-500">
            The retirement advice recommendations are very similar between both
            scenarios, indicating consistent strategic direction regardless of
            transitions.
          </p>
        </div>
      )}
