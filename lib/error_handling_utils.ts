...
export function validateMilestones(
  ...
  for (const milestone of milestones) {
    ...
    if (milestone.date && milestone.date > new Date()) {
      const yearsDiff = (milestone.date.getTime() - Date.now()) /
        (1000 * 60 * 60 * 24 * 365);
      if (yearsDiff > 50) {
        errors.push({
          code: "MILESTONE_FAR_FUTURE",
          message:
            `Milestone date is unusually far in the future: ${milestone.title}`,
          severity: "warning",
          context: {
  ...
  if (!advice.recommendations || !Array.isArray(advice.recommendations)) {
    ...
    for (const recommendation of advice.recommendations) {
      ...
      if (
        ...
        recommendation.effectivenessScore > 100
      ) {
        errors.push({
          code: "INVALID_EFFECTIVENESS_SCORE",
          message:
            `Recommendation has invalid effectiveness score: ${recommendation.effectivenessScore}`,
          severity: "warning",
          context: {
      ...
      if (
        recommendation.feasibilityScore < 0 ||
        recommendation.feasibilityScore > 100
      ) {
        errors.push({
          code: "INVALID_FEASIBILITY_SCORE",
          message:
            `Recommendation has invalid feasibility score: ${recommendation.feasibilityScore}`,
          severity: "warning",
          context: {
            recommendationId: recommendation.id,
 ...
 */
export function createEmptyMilestoneResult(
 ...
 */
export function createEmptyAdviceResult(
...
export function withGracefulDegradation<T, F>(
  ...
  try {
    ...
    console.warn(
      `Primary operation failed in ${context}, using fallback:`,
      error,
    );
...
export function sanitizeMilestoneForDisplay(milestone: Milestone): Milestone {
  return {
    ...
    date: milestone.date && !isNaN(milestone.date.getTime())
      ? milestone.date
      : new Date(),
    financialImpact: typeof milestone.financialImpact === "number" &&
        !isNaN(milestone.financialImpact)
 ...
 */
export function sanitizeAdviceForDisplay(
  advice: RetirementAdvice,
): RetirementAdvice {
  return {
    ...advice,
    ...
    recommendations: (advice.recommendations || []).map((rec) => ({
      ...
      specificActions: Array.isArray(rec.specificActions)
        ? rec.specificActions
        : [],
      effectivenessScore: typeof rec.effectivenessScore === "number" &&
          !isNaN(rec.effectivenessScore)
        ? Math.max(0, Math.min(100, rec.effectivenessScore))
        : 0,
      feasibilityScore:
        typeof rec.feasibilityScore === "number" && !isNaN(rec.feasibilityScore)
          ? Math.max(0, Math.min(100, rec.feasibilityScore))
          : 0,
    })),
    quickWins: Array.isArray(advice.quickWins) ? advice.quickWins : [],
    longTermStrategies: Array.isArray(advice.longTermStrategies)
      ? advice.longTermStrategies
      : [],
  };
}
