...
import type { TransitionTemplate, UserParameters } from "../types/financial.ts";
 ...
 * @param personId - Which household member the change is for, for templates
 *   whose math depends on that person's actual current values (e.g. a
 *   salary-based template in couple mode). Omit for a household-level
 *   template, or for an initial preview before the user has picked who -
 *   re-call with the chosen personId once they have, to get an accurate
 *   number rather than a household-level guess.
 * @returns Partial parameter changes generated by the template, or null if template not found
 *
 * @example
 * ```typescript
 * const changes = applyTemplate("semi-retirement", currentParams, personId);
 * if (changes) {
...
export function applyTemplate(
  ...
  return template.generateChanges(currentParams, personId);
}
