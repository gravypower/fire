...
Deno.test("MilestoneDetector - loan payoff interest is consistent regardless of simulation length", () => {
  ...
  const shortMilestones = new MilestoneDetector().detectLoanPayoffs(shortStates, params);
  const longMilestones = new MilestoneDetector().detectLoanPayoffs(longStates, params);
  ...
  assertEquals(shortMilestone.finalPaymentAmount, longMilestone.finalPaymentAmount);
