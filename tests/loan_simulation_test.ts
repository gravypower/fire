...
Deno.test("Single loan with offset - balance reduces faster", () => {
  ...
  const resultWithoutOffset = SimulationEngine.runSimulationWithTransitions(
    configWithoutOffset,
  );
  const resultWithOffset = SimulationEngine.runSimulationWithTransitions(
    configWithOffset,
  );
  ...
  assertEquals(
    month6WithOffset.loanBalance < month6WithoutOffset.loanBalance,
    true,
  );
  ...
  console.log(
    `  Without offset (month 6): $${
      month6WithoutOffset.loanBalance.toFixed(2)
    }`,
  );
  console.log(
    `  With offset (month 6): $${month6WithOffset.loanBalance.toFixed(2)}`,
  );
  console.log(
    `  Interest saved: $${month6WithOffset.interestSaved.toFixed(2)}`,
  );
});
...
Deno.test("Multiple loans - both balances reduce", () => {
  ...
  console.log(
    `  Home loan: $${initialLoan1.toFixed(2)} → $${month6Loan1.toFixed(2)}`,
  );
  console.log(
    `  Car loan: $${initialLoan2.toFixed(2)} → $${month6Loan2.toFixed(2)}`,
  );
  console.log(
    `  Total: $${initialState.loanBalance.toFixed(2)} → $${
      month6State.loanBalance.toFixed(2)
    }`,
  );
});
...
Deno.test("No loans - legacy fields ignored", () => {
  ...
  console.log(
    `  Initial loan balance: $${initialState.loanBalance.toFixed(2)}`,
  );
...
Deno.test("Multiple loans with different frequencies", () => {
  ...
  console.log(
    `  Monthly loan: $${initialLoan1.toFixed(2)} → $${month6Loan1.toFixed(2)}`,
  );
  console.log(
    `  Fortnightly loan: $${initialLoan2.toFixed(2)} → $${
      month6Loan2.toFixed(2)
    }`,
  );
});
